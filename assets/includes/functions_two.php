 ...
 * 
 * @global type $wo
 * @global type $sqlConnect
 * @param array $registration_data
 * @return boolean
 */
function Wo_RegisterCheckIn($registration_data = array()) {
    global $wo, $sqlConnect;
    if (empty($registration_data)) {
        return false;
    }
    ...
    $query = mysqli_query($sqlConnect, "INSERT INTO " . T_CHECK_INS . " ({$fields}) VALUES ({$data})");
    if ($query) {
        return mysqli_insert_id($sqlConnect);
    } else {
        return null;
 ...
 * @return boolean
 */
function Wo_CheckInDataById($id = false) {
    global $sqlConnect, $wo;
    if (empty($id) || !is_numeric($id) || $id < 0) {
        return false;
    }
    $data = array();
    $id = Wo_Secure($id);
    $query = mysqli_query($sqlConnect, "SELECT * FROM " . T_CHECK_INS . "  WHERE `id` = '$id'");
    $fetched_data = mysqli_fetch_assoc($query);
    if (!empty($fetched_data)) {
        return $fetched_data;
    }
...
/**
 * Check if lat lng user is exist
 * 
 * @global type $wo
 * @global type $sqlConnect
 * @param type $lat
 ...
 * @return boolean
 */
function Wo_IsLatLngCheckInExist($lat, $lng, $user_id) {
    global $wo, $sqlConnect;
    $data = array();
    if (empty($lat) || empty($lng) || empty($user_id)) {
        return false;
    }
    ...
    $query = mysqli_query($sqlConnect, "SELECT COUNT(`id`) as check_ins FROM " . T_CHECK_INS . " WHERE `lat` = '{$lat}' AND `lng` = '{$lng}' AND `user_id` = '{$user_id}'");
    $fetched_data = mysqli_fetch_assoc($query);
    if ($fetched_data['check_ins'] == 1) {
        return true;
    }
 ...
 * @return type
 */
function Wo_GetMyCheckIns() {
    global $sqlConnect, $wo;
    $user_id = Wo_Secure($wo['user']['user_id']);
    $data = array();
    $str_query = "(SELECT id, postText AS description, postMap AS `address`, check_in_name AS `name`, page_id, lat, lng, time, 'approve' AS `status` FROM " . T_POSTS . " WHERE postMap <> '' AND user_id = {$user_id})";
    $str_query .= " UNION ALL";
    $str_query .= " (SELECT id, description, address, `name`, page_id, lat, lng, time, `status` FROM " . T_CHECK_INS . " WHERE user_id = {$user_id} AND `status` <> 'reject' AND post_id IS NULL)";
    $str_query .= " ORDER BY time DESC";
    $query = mysqli_query($sqlConnect, $str_query);
    while ($fetched_data = mysqli_fetch_assoc($query)) {
        if (is_array($fetched_data)) {
            $data[] = $fetched_data;
        }
 ...
 * @return boolean
 */
function Wo_UpdateCheckIn($id = 0, $update_data = array()) {
    global $sqlConnect, $wo;
    $update = array();
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $impload = implode(', ', $update);
    $query_one = "UPDATE " . T_CHECK_INS . " SET {$impload} WHERE `id` = {$id} ";
 ...
 * @return boolean
 */
function Wo_GetCities() {
    global $sqlConnect, $wo;
    $data = array();
    $query = mysqli_query($sqlConnect, "SELECT * FROM " . T_ADMINISTRATIVES . " ORDER BY name ASC");
    while ($fetched_data = mysqli_fetch_assoc($query)) {
        if (is_array($fetched_data)) {
            $data[] = $fetched_data;
        }
...
/* 
Duy them   
Ham lay thong tin tu bang CustomField cho User + Thong tin User
*/

function Wo_GetFair($id = 0) {
    global $sqlConnect, $wo;
    if (empty($id)) {
        return false;
    }
    $id            = Wo_Secure($id);
    $sql_query_one = mysqli_query($sqlConnect, "SELECT * FROM " . T_USERS_FIELDS . " WHERE `user_id` = '$id'");
    $fetched_data  = mysqli_fetch_assoc($sql_query_one);

    if (!empty($fetched_data)) {
        $fetched_data['author']        = Wo_UserData($fetched_data['user']);
        $fetched_data['fid_3']     = Wo_UserFieldsData($fetched_data['fid_3']);
        
         $fetched_data['author']        = Wo_UserData($fetched_data['user_id']);
...
/* 
Duy them   
Ham lay thong tin tu bang Wo_ProfileFields
*/
function Wo_ProfileFields() {
    global $wo, $sqlConnect;
    $query_one    = "SELECT * FROM " . T_FIELDS ;
    $sql          = mysqli_query($sqlConnect, $query_one);
    $fetched_data = mysqli_fetch_assoc($sql);
    if (empty($fetched_data)) {
        return array();
